...
## scaffoldの境界（最重要）
- scaffoldは Header と BottomNav を自動で追加する。**contentFrameに絶対に追加してはいけない**
- contentFrameは「ヘッダーとナビの間のコンテンツ領域」だけを担当する
- Type A: contentFrameはy=110〜772の白背景スクロール領域
- Type B: contentFrameは全画面(0〜874)の背景と動画オーバーレイを担当

## スタイル参照画像の使い方（重要）
- 添付のスタイル参照画像は「色・グラデーション・フォントスタイル・コンポーネントの質感」だけを参考にする
- **レイアウト構造・要素の配置・画面の種類は仕様書に従う**（参照画像のレイアウトをコピーしない）
- 参照画像がType Bでも、今回の仕様がType Aなら白背景で生成する

## 設計参照画像の使い方
- 設計参照がある場合のみレイアウト・構造を参考にする
...
${figmaImageBase64 ? '## 参照デザイン画像\n添付の画像がターゲットデザインです。この外観を再現するコードを生成してください。' : ''}`;
  ...
  // Style references: color/tone only, NOT layout
  if (styleRefImages.length > 0) {
    contentBlocks.push({ type: 'text', text: `## スタイル参照（色・質感のみ参考にする、レイアウトはコピーしない）\n以下の画像からTwomiの色・グラデーション・フォントスタイル・コンポーネントの質感を読み取ってください。\n⚠️ これらのレイアウト・構造・画面タイプは無視すること。今回の仕様書に従って設計すること。` });
    for (const img of styleRefImages) {
      contentBlocks.push({ type: 'image', source: { type: 'base64', media_type: 'image/png', data: img } });
    }
  }

  // Specific design reference: layout reference
  if (figmaImageBase64) {
    contentBlocks.push({ type: 'text', text: '## 設計参照（レイアウト・構造を参考にする）\n以下の画像のレイアウトと構造を参考にしてください。' });
