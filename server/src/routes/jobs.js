...
router.post('/', async (req, res) => {
  const { userId, screenName, screenType, selectedTab, projectName, specUrl, specText, figmaRefUrl, product } = req.body;
  ...
  // PR #28: projectName routes the generated screen to a dedicated Figma page.
  // Optional — when blank, the scaffold falls back to figma.currentPage.
  const input = { screenName, screenType, selectedTab, projectName, specUrl, specText, figmaRefUrl, product };
  const { rows } = await query(
